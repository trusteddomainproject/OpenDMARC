...
**                                return a structure ARC specific parse
**                                result(especially client-ip)
**
** Parameters:
** 	aar -- a parse result structure parsed by opendmarc_arcares_parse
                             ...
                             struct arcares_arc_field *arc)
{
	...
	for (cr = 0; cr < aar->payload.ares_count; cr++)
	{
		if (aar->payload.ares_result[cr].result_method == ARES_METHOD_ARC)
		{
			...
			{
			    /* old implementation of OpenARC used to use "client-ip" */
				if (  (arc->arcresult.result_ptype[cp] == ARES_PTYPE_SMTP)
				    &&(  (strcasecmp(arc->arcresult.result_property[cp],
				                     "remote-ip"))
                       ||(strcasecmp(arc->arcresult.result_property[cp],
				                     "client-ip"))))
				{
					...
					strlcpy(arc->smtpclientip, arc->arcresult.result_value[cp],
