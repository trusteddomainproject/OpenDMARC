...
- **Subdomain DMARC fallback to parent domain**: When a subdomain had no `_dmarc` record, OpenDMARC re-queried the same subdomain rather than walking up to the parent, producing `dmarc=none` even when the parent domain published a policy. Two-part fix: (1) the PSL path now correctly detects when the PSL returns the input domain unchanged (no PSL configured, or PSL could not identify a boundary) rather than treating it as a valid org-domain result; (2) when the PSL is absent, a label-walk fallback queries successive parent labels until a DMARC record is found, stopping before bare TLDs. A `LOG_WARNING` is emitted when the fallback fires, directing operators to configure `PublicSuffixList` for RFC 7489-compliant behavior. Adds `opendmarc_policy_fetch_org_domain_from_fallback()` public accessor. Note: the fallback will be superseded by the RFC 9989 DNS tree walk before release. (#378, issue #54)
